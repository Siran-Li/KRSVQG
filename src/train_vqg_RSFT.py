...
from models.blip_vqg_c2q_onlyqg import blip_vqg_c2q_onlyqg
import utils
...
def train(model, data_loader, optimizer, epoch, device):
    ...
    for i,(image, caption, question, triplet, n, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss = model(image, triplet, caption, question, train=True, n=n)        
        
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()    

        metric_logger.update(loss_qg=loss.item())
...
def main(args, config):
    ...
    model = blip_vqg_c2q_onlyqg(pretrained=config['pretrained'], pre_cgmodel=config['pre_cgmodel'], freeze_img=args.freeze_img,
    ...
    if args.load_optimizer:
        optimizer.load_state_dict(torch.load(config['pretrained'])['optimizer'])
    ...
    for epoch in range(0, config['max_epoch']):
        ...
        if utils.is_main_process():   
            if config['dataset'] == 'nwpu':
                coco_val = rs_caption_eval(config['ann_root'],val_result_file,'val_nwpu')
                coco_val_qg = rs_question_eval(config['ann_root'],val_result_file,'val_nwpu')
            elif config['dataset'] == 'textrs':
                coco_val = rs_caption_eval(config['ann_root'],val_result_file,'val_textrs')
                coco_val_qg = rs_question_eval(config['ann_root'],val_result_file,'val_textrs')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/vqg_kvqg_nwpu.yaml') 
    parser.add_argument('--output_dir', default='output/NWPU-300/Model3')
    ...
    parser.add_argument('--freeze_img', action='store_true', default=True)   
