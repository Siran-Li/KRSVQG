...
from data.utils_nwpu import save_result, nwpu_caption_eval, nwpu_question_eval
...
def main(args, config):
    ...
    for epoch in range(0, config['max_epoch']):
        ...
        if utils.is_main_process():   
            coco_val = nwpu_caption_eval(config['ann_root'],val_result_file,'val_ctq')
            coco_val_qg = nwpu_question_eval(config['ann_root'],val_result_file,'val_ctq')
